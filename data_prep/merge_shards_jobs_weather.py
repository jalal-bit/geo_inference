...
def merge_shards( args):
    ...
    for csv_path in csvs:
        ...
        shard_paths = sorted([p for p in glob.glob(f"{base_no_ext}.shard_*.csv") if not p.endswith(PRED_SUFFIX)])
        ...
        for sp, pp in zip(shard_paths, pred_paths):
            if pp.exists():
                ...
                pf = pd.DataFrame([{"is_weather": 0}] * shard_len)
            preds_list.append(pf)

        merged_preds = pd.concat(preds_list, ignore_index=True)
        df_out = df.copy()
        df_out["is_weather"] = merged_preds["is_weather"].astype(int)
