...
def prompt_is_weather(text: str) -> str:
    return f"""
You are a classifier. You will classify if a text is a weather report.
Here is an example:

Text: "A severe thunderstorm watch has been issued for parts of Oklahoma until 4 AM CDT"
Answer: {{"is_weather": 1}}

Now classify the following text. Answer only with a valid JSON object {{"is_weather": 1}} if yes, or {{"is_weather": 0}} if no.
...
def classify_batch(batch_texts, model, tokenizer, gen_kwargs, accelerator):
    ...
    # ---- Weather
    weather_prompts = [prompt_is_weather(t) for t in batch_texts]
    weather_tok = tokenizer(weather_prompts, return_tensors="pt", padding=True, truncation=True, max_length=512).to(model.device)
    weather_out = unwrapped_model.generate(**weather_tok, **gen_kwargs)
    weather_decoded = tokenizer.batch_decode(weather_out, skip_special_tokens=True)
    for d in weather_decoded:
        results["is_weather"].append(safe_json_parse(d, "is_weather"))
...
def worker_loop(accelerator, args):
    ...
    for shard_path in shards:
        ...
        print(f"[rank {rank}] wrote {pred_path} ({len(outputs['is_weather'])} rows)")
