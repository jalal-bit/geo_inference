#!/usr/bin/env python3
"""
Geo-only evaluation (4-bit capable) with geo-only error analysis.
...
This version:
  ...
  - supports base model, full FT checkpoint, and LoRA/QLoRA adapter checkpoint
  - supports 4-bit base loading for adapter evaluation
...
def extract_last_text_block(prompt: str) -> Optional[str]:
    ...
    if matches:
        txt = matches[-1].group(1).strip().strip('"').strip()
        if txt:
            return txt

    idx = s.rfind("Text:")
    if idx != -1:
        txt = s[idx + len("Text:"):].strip().strip('"').strip()
        if txt:
            return txt
    ...
    def __init__(self, csv_path: str, prompt_col="prompt", target_col="target_json"):
        ...
        for p, t in zip(prompts, targets):
            ...
            self.examples.append({
                "prompt": maybe_rewrite_prompt_to_geo_only(p),
                "target": target_json_to_geo_only_json(t),
            })
...
def postprocess_pred_text_geo_only(raw: str) -> Tuple[str, Dict[str, Any]]:
    ...
    info["mode"] = "repetition" if info["arrays_unique"] == 1 and info["arrays_found"] > 1 else ("single" if info["arrays_unique"] == 1 else "multi")
...
def compute_test_metrics_geo_only(pred_texts: List[str], gold_texts: List[str]) -> Dict[str, float]:
    ...
    abs_err_sum = bias_sum = exact_count_hits = pred_count_sum = gold_count_sum = 0.0
    ...
    def micro_prf_from_sets(pred_sets, gold_sets):
        tp = fp = fn = 0.0
    ...
    geo_tp = geo_fp = geo_fn = geo_em_hits = 0.0
    ...
    return {
        ...
        "entity_count_mae": float(abs_err_sum / n),
        "entity_count_bias": float(bias_sum / n),
        "entity_count_exact_match": float(exact_count_hits / n),
        "pred_entity_avg": float(pred_count_sum / n),
        "gold_entity_avg": float(gold_count_sum / n),
    }
...
def run_error_analysis(
    ...
    pd.DataFrame(rows).to_csv(out_dir / f"{base}_rows.csv", index=False)
    pd.DataFrame(state_rows).to_csv(out_dir / f"{base}_state_errors.csv", index=False)
    pd.DataFrame(county_rows).to_csv(out_dir / f"{base}_county_errors.csv", index=False)
    pd.DataFrame(city_rows).to_csv(out_dir / f"{base}_city_errors.csv", index=False)
    pd.DataFrame(full_rows).to_csv(out_dir / f"{base}_full_errors.csv", index=False)

    _entity_prf_by_item(pred_state_sets, gold_state_sets).to_csv(out_dir / f"{base}_entity_prf_by_item_state.csv", index=False)
    _entity_prf_by_item(pred_county_sets, gold_county_sets).to_csv(out_dir / f"{base}_entity_prf_by_item_county.csv", index=False)
    _entity_prf_by_item(pred_city_sets, gold_city_sets).to_csv(out_dir / f"{base}_entity_prf_by_item_city.csv", index=False)
...
def main():
    p = argparse.ArgumentParser(description="Geo-only 4-bit evaluation with geo-only error analysis")
