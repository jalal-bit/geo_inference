...
Original prompt may mention evidence.
...
This script converts them on the fly to:
Prompt:
  geo-only instruction + original post text
...
def extract_last_text_block(prompt: str) -> Optional[str]:
    """
    Recover the actual social media post from an evidence-based prompt.
    Uses the final `Text: ... Output:` block, or the final `Text:` block.
    """
    if prompt is None:
        return None
...
def maybe_rewrite_prompt_to_geo_only(prompt: str) -> str:
    """
    Rewrite evidence-based prompt into geo-only prompt while preserving the post text.
    If prompt already looks geo-only, return it unchanged.
    """
    if prompt is None:
        return ""
    ...
    lower_s = s.lower()
    if (
        '"evidence"' not in lower_s
        and "evidence" not in lower_s
        and '"mention"' not in lower_s
        and "mention" not in lower_s
    ):
        return s
...
def cast_trainable_params_to_bf16(model):
    """
    Some trainable params can remain fp32 after PEFT wrapping.
    Cast trainable fp32 params to bf16.
    """
    target_dtype = torch.bfloat16
    for name, param in model.named_parameters():
        if param.requires_grad and param.dtype == torch.float32:
            print(f"[cast fp32] {name}: {param.dtype} -> {target_dtype}")
...
def entity_geo_key(ent: Dict[str, Any]) -> Optional[Tuple[str, str, str]]:
    ...
    Geo key:
      ...
      - else -> (state, county, "")
    Requires at least state+county.
    """
...
# Checkpoint saving
# --------------------------
def save_model_checkpoint(accelerator, model, tokenizer, out_dir: str):
    """
    Save checkpoint safely for plain, LoRA, and FSDP-wrapped models.
    """
    os.makedirs(out_dir, exist_ok=True)

    unwrapped = accelerator.unwrap_model(model)

    # PEFT models know how to save adapters; full models save full weights.
...
def train_loop(args, train_ds: Dataset, val_ds: Dataset):
    ...
    if args.fsdp:
        fsdp_plugin = FullyShardedDataParallelPlugin(
            ...
            cpu_offload=False,
            auto_wrap_policy="TRANSFORMER_BASED_WRAP",  # replaced after PEFT wrapping if needed
    ...
    # Required for PEFT + FSDP auto wrapping
    ...
    if accelerator.is_main_process:
        ...
        if args.wandb_project and wandb_key:
            ...
            wandb.init(
                ...
                config=vars(args),
                name=f"{safe_model_name}_{args.timestamp}",
            )
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train instruction->JSON entity-array model (geo-only, QLoRA/FSDP capable)"
    )
