...
class DataCollatorForCausalLM:
    ...
    def __call__(self, batch):
        ...
        # Build labels, mask out prompt
        labels = input_ids.clone()
        for i, pl in enumerate(prompt_lens):
            labels[i, :pl] = -100  # ignore prompt in loss
...
# --------------------------
def evaluate(model, val_loader, tokenizer, accelerator, gen_cfg=None, log_first_batch=False):
    """
    Multi-GPU safe:
    - generate locally
    ...
    """
    model.eval()
    gen_cfg = gen_cfg or {"num_beams": 1, "do_sample": False, "max_new_tokens": 60}
    ...
    with torch.no_grad():
        for step, batch in enumerate(val_loader):
            ...
            gathered_preds = accelerator.gather_object(local_pred_texts)
            gathered_golds = accelerator.gather_object(local_gold_texts)
...
def train_loop(model_name,train_dataset,val_dataset, hf_token,batch_size, check_point_path, epochs=3,lr=2e-5,weight_decay=0.01,warmup_ratio=0.03,grad_accum_steps=1,max_grad_norm=1.0,gen_cfg=None,save_best_on="county_f1",wandb_project=None,wandb_key=None, wandb_config=None):
    ...
    if accelerator.is_main_process:
        ...
        # Convert to tensor of uint8
        t_bytes = timestamp.encode("utf-8")
        data_to_broadcast = torch.tensor(list(t_bytes), dtype=torch.uint8, device=accelerator.device)
    else:
        # Placeholder tensor; other processes will receive broadcast
        data_to_broadcast = torch.zeros(64, dtype=torch.uint8, device=accelerator.device)  # max 64 chars

    # Broadcast from main process to all other processes
    received_tensor = broadcast(data_to_broadcast, from_process=0)

    # Convert back to string and strip padding zeros
    timestamp_str = bytes(received_tensor.tolist()).decode("utf-8").rstrip("\x00")
    """
    ...
    if accelerator.is_main_process:
        ...
        if wandb_project and wandb_key and wandb_config:
            ...
            # wandb.init(project=wandb_project,config=wandb_config,name=timestamp_str)
    ...
    for epoch in range(epochs):
        ...
        if accelerator.is_main_process:
            global_loss = sum(gathered_loss)
            global_cnt  = sum(gathered_cnt)
            avg_loss = global_loss / max(1, global_cnt)
            print(f"Epoch {epoch+1} | Train Loss: {avg_loss:.6f} | elapsed {time.time()-start:.1f}s")
            if wandb_project and wandb_key and wandb_config:
                wandb.log({"train/loss": avg_loss})

        # Validation
        metrics = evaluate(model, val_loader, tokenizer, accelerator, gen_cfg=gen_cfg, log_first_batch=True)
        ...
        if accelerator.is_main_process:
            ...
            if wandb_project and wandb_key and wandb_config:
                wandb.log({"valid/county_f1": score})
                
                county_acc=metrics.get("county_acc", -1.0)
                wandb.log({"valid/county_acc": county_acc})
                
                county_precision=metrics.get("county_precision", -1.0)
                wandb.log({"valid/county_precision": county_precision})
                
                state_f1=metrics.get("state_f1", -1.0)
                wandb.log({"valid/state_f1": state_f1})
                
                state_precision=metrics.get("state_precision", -1.0)
                wandb.log({"valid/state_precision": state_precision})
                
                state_acc=metrics.get("state_acc", -1.0)
                wandb.log({"valid/state_acc": state_acc})
                
                us_f1=metrics.get("us_f1", -1.0)
                wandb.log({"valid/us_f1": us_f1})
                
                us_acc=metrics.get("us_acc", -1.0)
                wandb.log({"valid/us_acc": us_acc})
                
                us_precision=metrics.get("us_precision", -1.0)
                wandb.log({"valid/us_precision": us_precision})
...
def main_cli():
    ...
    if train_ds!=None and val_ds!=None:
       ...
       train_loop(args.model_name,train_ds,val_ds,args.hf_token,args.batch_size,args.check_point_path, args.epochs, args.lr, args.weight_decay, args.warmup_ratio, args.grad_accum_steps, args.max_grad_norm, args.gen_cfg, args.save_best_on, args.wandb_project, args.wandb_key,config_dict,)
